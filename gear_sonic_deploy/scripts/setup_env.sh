...
echo "🔧 Removing ROS2 library paths to prevent CycloneDDS conflict..."
export LD_LIBRARY_PATH=$(echo "$LD_LIBRARY_PATH" | tr ':' '\n' | grep -v "ros2" | grep -v "ros2_humble" | tr '\n' ':' | sed 's/:$//')
echo "✅ ROS2 library paths removed from LD_LIBRARY_PATH"
...
export LD_LIBRARY_PATH=/opt/TensorRT/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH
export CYCLONEDDS_URI='<CycloneDDS><Domain><General><Interfaces><NetworkInterface name="lo" multicast="true"/></Interfaces><EnableMulticastLoopback>true</EnableMulticastLoopback></General></Domain></CycloneDDS>'
